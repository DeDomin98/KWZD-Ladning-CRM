...
const CrmLayout = ({ department }) => {
  ...
  const auth = getAuth();
  const { userData, displayName, isRestricted, isNegocjacjeOnly, canSeeDepartment, canSeeFinances, canSeeSettings, canSeeAllLeads } = useAuth();
  ...
  const totalUnread = Object.values(unreadCounts).reduce((a, b) => a + b, 0) + unreadGroupCount + unreadNegocjacjeCount;
